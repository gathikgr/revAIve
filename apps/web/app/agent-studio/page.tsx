    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 font-sans bg-[#0a2540] text-slate-100">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <form onSubmit={handleRunAgent} className="bg-[#1a1f36] border border-[#2a2f45] rounded-2xl p-6 space-y-5 shadow-2xl">
            <div className="border-b border-[#2a2f45] pb-3">
              <h2 className="text-sm font-extrabold text-white uppercase tracking-wider">1. Merchant & Customer Context</h2>
            </div>

            <div className="space-y-3 text-xs">
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Merchant Organization Name</label>
                <input
                  type="text"
                  required
                  value={merchantName}
                  onChange={(e) => setMerchantName(e.target.value)}
                  className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-white font-medium focus:outline-none focus:border-[#635bff]"
                />
              </div>

              <div>
                <label className="block text-slate-300 font-semibold mb-1">Razorpay Account ID</label>
                <input
                  type="text"
                  required
                  value={razorpayMerchantId}
                  onChange={(e) => setRazorpayMerchantId(e.target.value)}
                  className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-slate-300 font-mono text-[11px]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-white"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-white font-mono text-[11px]"
                  />
                </div>
              </div>
            </div>

            <div className="border-b border-[#2a2f45] pb-3 pt-2">
            ...
            <div className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Amount at Risk (₹ INR)</label>
                  <input
                    type="number"
                    step="1"
                    min="1"
                    ...
                    className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-white font-bold font-mono"
                  />
                </div>
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Issuing Bank</label>
                  <select
                    value={issuerBank}
                    onChange={(e) => setIssuerBank(e.target.value)}
                    className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-white font-medium"
                  >
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Failure Reason Code</label>
                <select
                  value={failureCode}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-white capitalize"
                  >
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Previous Retries</label>
                  <input
                    type="number"
                    min="0"
                    max="5"
                    ...
                    className="w-full bg-[#0a2540] border border-[#2a2f45] rounded-xl p-2.5 text-white font-mono"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 pt-2">
                <input
                  ...
                  className="rounded bg-[#0a2540] border-[#2a2f45] text-[#635bff] focus:ring-0"
                />
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-[#635bff] hover:bg-[#544dc9] text-white font-bold text-xs rounded-xl shadow-xl transition-all flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
        ...
        <div className="lg:col-span-7 space-y-6">
          {!reviewResult && !loading && (
            <div className="bg-[#1a1f36]/60 border border-[#2a2f45] border-dashed rounded-2xl p-12 text-center space-y-3">
              <div className="w-12 h-12 bg-[#0a2540] rounded-2xl flex items-center justify-center text-2xl mx-auto text-slate-400">
                🔍
              </div>
          ...
          {loading && (
            <div className="bg-[#1a1f36] border border-[#2a2f45] rounded-2xl p-12 text-center space-y-4 shadow-xl">
          ...
          {reviewResult && !loading && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="bg-[#1a1f36] border border-[#635bff]/40 rounded-2xl p-6 shadow-2xl space-y-4">
                <div className="flex justify-between items-center border-b border-[#2a2f45] pb-3">
                  <div>
                  ...
                  <div className="text-right font-mono">
                    <span className="text-[10px] text-slate-400 block">Pipeline Latency</span>
                    <span className="text-xs text-[#00d4b2] font-bold">{reviewResult.scenario_summary.latency_ms} ms</span>
                  </div>
                </div>

                <div className="grid grid-cols-3 gap-3 text-xs font-mono text-center">
                  <div className="bg-[#0a2540] p-3 rounded-xl border border-[#2a2f45]">
                    <div className="text-slate-400 text-[10px]">Amount At Risk</div>
                    <div className="text-white font-bold text-sm mt-1">{reviewResult.scenario_summary.amount_at_risk}</div>
                  </div>
                  <div className="bg-[#0a2540] p-3 rounded-xl border border-[#2a2f45]">
                    <div className="text-slate-400 text-[10px]">Expected Recovery</div>
                    <div className="text-[#00d4b2] font-bold text-sm mt-1">{reviewResult.scenario_summary.expected_recovery_value}</div>
                  </div>
                  <div className="bg-[#0a2540] p-3 rounded-xl border border-[#2a2f45]">
                    <div className="text-slate-400 text-[10px]">Opportunity Status</div>
                    <div className="text-[#635bff] font-bold text-sm mt-1 uppercase">{reviewResult.scenario_summary.status}</div>
                  </div>
                </div>
              </div>

              {/* Where Revenue is Leaking Card (Diagnosis) */}
              <div className="bg-[#1a1f36] border border-[#2a2f45] rounded-2xl p-6 space-y-4 shadow-xl">
                <div className="flex justify-between items-center border-b border-[#2a2f45] pb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-lg">🔎</span>
                    <h3 className="text-sm font-extrabold text-white uppercase tracking-wider">Where Revenue is Leaking</h3>
                  </div>
                  <span className="px-2.5 py-0.5 text-[10px] font-bold rounded bg-[#635bff]/20 text-[#00d4b2] border border-[#635bff]/30 font-mono">
                    Diagnostic Confidence: {reviewResult.detailed_review.leakage_diagnosis.confidence}
                  </span>
                </div>

                <div className="space-y-3 text-xs">
                  <div className="p-4 bg-[#0a2540] rounded-xl border border-[#2a2f45]">
                    <div className="text-[10px] text-slate-400 uppercase font-mono font-bold">Root Cause Category</div>
                    <div className="text-base font-black text-[#635bff] mt-0.5">
              ...
              <div className="bg-[#1a1f36] border border-[#2a2f45] rounded-2xl p-6 space-y-4 shadow-xl">
                ...
                <div className="space-y-2 text-xs font-mono">
                  {reviewResult.detailed_review.policy_guard_gate.guard_rule_breakdown.map((rule: string, i: number) => (
                    <div key={i} className="p-3 bg-[#0a2540] rounded-xl border border-[#2a2f45] flex justify-between items-center">
                      <span className="text-slate-300">{rule}</span>
                      <span className="text-[#00d4b2] font-bold">✓ VERIFIED</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-[#1a1f36] border border-[#2a2f45] rounded-2xl p-6 space-y-4 shadow-xl">
                <div className="flex justify-between items-center border-b border-[#2a2f45] pb-3">
                  ...
                  <span className="text-xs font-mono text-[#00d4b2] font-bold">
                ...
                <div className="p-3 bg-[#0a2540] rounded-xl border border-[#2a2f45] text-xs text-slate-300">
                  <div className="text-[10px] text-slate-400 font-mono uppercase font-bold mb-1">Strategy Rationale</div>
